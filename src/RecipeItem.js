...
const RecipeItem = ({item}) => {
  return (
    <div className='flex flex-col w-80 overflow-hidden p-5 bg-white/75 shadow-xl gap-5 border-2 rounded-2xl border-white'>
       <div className='h-40 flex justify-center overflow-hidden items-center reounded-xl'>
       ...
       <div>
         ...
         <Link className='text-s, p-3 mt-5 px-8 rounded-lg uppercase font-medium tracking-wider inline-block shadow-md bg-black text-white' 
               to={`RecipeItem/${item.id}`}
           >Recipe Details
         </Link>
       </div>
    </div>
